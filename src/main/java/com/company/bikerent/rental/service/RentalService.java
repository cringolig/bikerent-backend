...
public class RentalService {
    ...
    public RentalDto create(CreateRentalRequest request) {
        log.info("Starting rental: user={}, bicycle={}, station={}", 
                request.userId(), request.bicycleId(), request.startStationId());
        ...
        Station startStation = stationRepository.findById(request.startStationId())
                .orElseThrow(() -> new EntityNotFoundException(Station.class, request.startStationId()));
    ...
    public RentalDto complete(Long rentalId, CompleteRentalRequest request) {
        ...
        Rental rental = rentalRepository.findByIdWithLock(rentalId)
